...
static UART_HandleTypeDef s_tUART = {USART1};
...
/***************************************************************************************************
 * @fn      UART_INIT_DEF(uart1)()
 *
 * @brief   uart1 interface init
 *
 * @author  chuanpengl
 *
 * @param   none
 ...
 * @return  none
 */
void UART_INIT_DEF(uart1)( uartHandle_t *a_ptUartHdl, iuUartPara_t *a_ptUartPara )
{
    GPIO_InitTypeDef tGpio_Init;

    s_ptUartHandle = a_ptUartHdl;
    s_hInd = a_ptUartPara->hInd;
    s_tUART.Instance = USART1;
    ...
    HAL_NVIC_SetPriority( USART1_IRQn, 0, 1 );    /* configLIBRARY_LOWEST_INTERRUPT_PRIORITY */

    HAL_NVIC_EnableIRQ( USART1_IRQn );

    __GPIOA_CLK_ENABLE();
    /* Configure USART Tx as alternate function push-pull, PA9 */
    tGpio_Init.Mode = GPIO_MODE_AF_PP;
    tGpio_Init.Pin = GPIO_PIN_9;
    tGpio_Init.Speed = GPIO_SPEED_HIGH;
    HAL_GPIO_Init(GPIOA, &tGpio_Init);

    /* Configure USART Rx as input floating, PA10 */
 ...
 * @return  none
 */
void UART_EXIT_DEF(uart1)( void *a_ptUart )
{
    GPIO_InitTypeDef tGpio_Init;
...
/***************************************************************************************************
 * @fn      UART_SEND_BYTE_WITH_IT_DEF(uart1)()
 *
 ...
 * @return  none
 */
uartBool UART_SEND_BYTE_WITH_IT_DEF(uart1)( uartU8 a_u8Byte )
{
 ...
 * @return  none
 */
uartU8 UART_EVENT_DEF(uart1)(  uartHandle_t *a_ptUart, uartU8 a_u8Evt )
{
    ...
    return 0;
}   /* UART_EVENT_DEF(uart1)() */

/***************************************************************************************************
 * @fn      UART_TX_CPLT_CALLBACK_DEF(uart1)()
 *
 ...
 * @return  none
 */
void UART_TX_CPLT_CALLBACK_DEF(uart1)(UART_HandleTypeDef *huart)
{
    if ( uartTrue == uart_GetSendByte( s_ptUartHandle, &gs_UartTxByte ) )
    {
        HAL_UART_Transmit_IT( &s_tUART, (uint8_t*)&gs_UartTxByte, 1 );
    }
}   /* UART_TX_CPLT_CALLBACK_DEF(uart1) */

/***************************************************************************************************
 * @fn      UART_RX_CPLT_CALLBACK_DEF(uart1)()
 *
 * @brief   dtu uart tx cplt callback, this is weak in uart driver, so this should not modify
 *
 * @author  chuanpengl
 ...
 * @return  none
 */
void UART_RX_CPLT_CALLBACK_DEF(uart1)(UART_HandleTypeDef *huart)
{
    uart_SetReceiveByte( s_ptUartHandle, gs_UartRxByte );
    HAL_UART_Receive_IT( &s_tUART, &gs_UartRxByte, 1 );
}   /* UART_RX_CPLT_CALLBACK_DEF(uart1) */

/***************************************************************************************************
 * @fn      UART_ERROR_CPLT_CALLBACK_DEF(uart1)()
 *
 ...
 * @return  none
 */
void UART_ERROR_CPLT_CALLBACK_DEF(uart1)(UART_HandleTypeDef *huart)
{
