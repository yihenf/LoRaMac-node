...
static UART_HandleTypeDef s_tUART = {USART3};
...
void UART_INIT_DEF(uart3)( uartHandle_t *a_ptUartHdl, iuUartPara_t *a_ptUartPara )
{
    ...
    HAL_NVIC_SetPriority( USART3_IRQn, 0, 1 );    /* configLIBRARY_LOWEST_INTERRUPT_PRIORITY */

    HAL_NVIC_EnableIRQ( USART3_IRQn );

    __GPIOB_CLK_ENABLE();
    /* Configure USART Tx as alternate function push-pull, PB10 */
    tGpio_Init.Mode = GPIO_MODE_AF_PP;
    tGpio_Init.Pin = GPIO_PIN_10;
    tGpio_Init.Speed = GPIO_SPEED_HIGH;
    HAL_GPIO_Init(GPIOB, &tGpio_Init);

    /* Configure USART Rx as input floating, PB11 */
...
}   /* UART_TX_CPLT_CALLBACK_DEF(uart3) */
...
void UART_RX_CPLT_CALLBACK_DEF(uart3)(UART_HandleTypeDef *huart)
{
    ...
    HAL_UART_Receive_IT( &s_tUART, &gs_UartRxByte, 1 );
}   /* UART_RX_CPLT_CALLBACK_DEF(uart3) */
