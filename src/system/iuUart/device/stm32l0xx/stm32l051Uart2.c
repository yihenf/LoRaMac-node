...
void UART_INIT_DEF(uart2)( uartHandle_t *a_ptUartHdl, iuUartPara_t *a_ptUartPara )
{
    ...
    tGpio_Init.Speed     = GPIO_SPEED_FAST;
    tGpio_Init.Alternate = GPIO_AF4_USART2;
    ...
    s_tUART.Init.OneBitSampling = UART_ONEBIT_SAMPLING_DISABLED;
    s_tUART.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
    ...
    HAL_NVIC_SetPriority( USART2_IRQn, 0, 0 );    /* configLIBRARY_LOWEST_INTERRUPT_PRIORITY */
...
uartU8 UART_EVENT_DEF(uart2)(  uartHandle_t *a_ptUart, uartU8 a_u8Evt )
{
    ...
    if ( UART_EVT_IDLE == ( a_u8Evt & UART_EVT_IDLE ) ){
        s_hInd( UART_IND_IDLE, 0 );
    }
...
void UART_TX_CPLT_CALLBACK_DEF(uart2)(UART_HandleTypeDef *huart)
{
    ...
    {
        uart_SetTxEmptyFlag( s_ptUartHandle );
    }
...
{
    rxTemp[rxTempIdx ++] = gs_UartRxByte;
